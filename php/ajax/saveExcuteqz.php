...
for($i=0;$i<=$lenght-1;$i++){
    ...
    {
        $sql="insert into topic (topic_fullmark,topic_tclass_no,topic_type,topic_times) VALUES ('{$de_json[0]['fullmark']}','{$de_json[0]['Tclass_no']}','期中考试','$time')";
        // echo $sql;
		$D->exec($sql);
        $num=select('v_homework1','*',"Tclass_no='{$de_json[$i]['Tclass_no']}' and topic_times='{$time}' and topic_type='期中考试'",$D);
		 foreach ($num as $a){
			$topic_no=$a['topic_no'];
		}
		// echo $num['topic_no']; 
